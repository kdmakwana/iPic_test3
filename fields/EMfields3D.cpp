...
void EMfields3D::MaxwellSource(double *bkrylov, Grid * grid, VirtualTopology3D * vct, Collective *col) {
  ...
  //kdm addition 16-05-2016
  if (Case=="AMRVAC")    fixBAMRVAC(grid,vct); 
  ...
  //kdm addition 06-05-2016
  // boundary condition: Xleft
  if (vct->getXleft_neighbor() == MPI_PROC_NULL && bcEMfaceXleft == 5)  // perfect conductor
    perfectAMRVACLeftS(tempX, tempY, tempZ, grid, 0);
  // boundary condition: Yleft
  if (vct->getYleft_neighbor() == MPI_PROC_NULL && bcEMfaceYleft == 5)  // perfect conductor
    perfectAMRVACLeftS(tempX, tempY, tempZ, grid, 1);
  // boundary condition: Zleft
  if (vct->getZleft_neighbor() == MPI_PROC_NULL && bcEMfaceZleft == 5)  // perfect conductor
    perfectAMRVACLeftS(tempX, tempY, tempZ, grid, 2);
  // boundary condition: Xright
  if (vct->getXright_neighbor() == MPI_PROC_NULL && bcEMfaceXright == 5)  // perfect conductor
    perfectAMRVACRightS(tempX, tempY, tempZ, grid, 0);
  // boundary condition: Yright
  if (vct->getYright_neighbor() == MPI_PROC_NULL && bcEMfaceYright == 5)  // perfect conductor
    perfectAMRVACRightS(tempX, tempY, tempZ, grid, 1);
  // boundary condition: Zright
  if (vct->getZright_neighbor() == MPI_PROC_NULL && bcEMfaceZright == 5)  // perfect conductor
    perfectAMRVACRightS(tempX, tempY, tempZ, grid, 2);
...
void EMfields3D::MaxwellImage(double *im, double *vector, Grid * grid, VirtualTopology3D * vct) {
  ...
  //kdm addition 13-05-2016 for AMRVAC boundary conditions
  // boundary condition: Xleft
  if (vct->getXleft_neighbor() == MPI_PROC_NULL && bcEMfaceXleft == 5) 
    perfectAMRVACLeft(imageX, imageY, imageZ, vectX, vectY, vectZ, 0, grid);
  // boundary condition: Xright
  if (vct->getXright_neighbor() == MPI_PROC_NULL && bcEMfaceXright == 5)
    perfectAMRVACRight(imageX, imageY, imageZ, vectX, vectY, vectZ, 0, grid);
  // boundary condition: Yleft
  if (vct->getYleft_neighbor() == MPI_PROC_NULL && bcEMfaceYleft == 5)  
    perfectAMRVACLeft(imageX, imageY, imageZ, vectX, vectY, vectZ, 1, grid);
  // boundary condition: Yright
  if (vct->getYright_neighbor() == MPI_PROC_NULL && bcEMfaceYright == 5) 
    perfectAMRVACRight(imageX, imageY, imageZ, vectX, vectY, vectZ, 1, grid);
  // boundary condition: Zleft
  if (vct->getZleft_neighbor() == MPI_PROC_NULL && bcEMfaceZleft == 5)  
    perfectAMRVACLeft(imageX, imageY, imageZ, vectX, vectY, vectZ, 2, grid);
  // boundary condition: Zright
  if (vct->getZright_neighbor() == MPI_PROC_NULL && bcEMfaceZright == 5) 
    perfectAMRVACRight(imageX, imageY, imageZ, vectX, vectY, vectZ, 2, grid);
...
//kdm addition 16-05-2016 fix B for AMRVAC
void EMfields3D::fixBAMRVAC(Grid * grid, VirtualTopology3D * vct) {

  cout << " kdm inside fixBAMRVAC " << endl; 
  int mhdsuccess;
  double xx,yy,zz,dval;
  amrvac_interf* amrvac_obj = new amrvac_interf();
  mhdsuccess = amrvac_obj->init_sim_box();
  mhdsuccess = amrvac_obj->fields_from_amrvac(); 

  if (vct->getZright_neighbor()==MPI_PROC_NULL){
    for (int i=0; i < nxc;i++)
      for (int j=0; j < nyc;j++){

        Bxc[i][j][nzc-1] = amrvac_obj->getbx_mhd(grid->getXC(i,j,nzc-1),grid->getYC(i,j,nzc-1),grid->getZC(i,j,nzc-1));
        Bxc[i][j][nzc-2] = amrvac_obj->getbx_mhd(grid->getXC(i,j,nzc-2),grid->getYC(i,j,nzc-2),grid->getZC(i,j,nzc-2));
        Bxc[i][j][nzc-3] = amrvac_obj->getbx_mhd(grid->getXC(i,j,nzc-3),grid->getYC(i,j,nzc-3),grid->getZC(i,j,nzc-3));

        Byc[i][j][nzc-1] = amrvac_obj->getby_mhd(grid->getXC(i,j,nzc-1),grid->getYC(i,j,nzc-1),grid->getZC(i,j,nzc-1));
        Byc[i][j][nzc-2] = amrvac_obj->getby_mhd(grid->getXC(i,j,nzc-2),grid->getYC(i,j,nzc-2),grid->getZC(i,j,nzc-2));
        Byc[i][j][nzc-3] = amrvac_obj->getby_mhd(grid->getXC(i,j,nzc-3),grid->getYC(i,j,nzc-3),grid->getZC(i,j,nzc-3));

        Bzc[i][j][nzc-1] = amrvac_obj->getbz_mhd(grid->getXC(i,j,nzc-1),grid->getYC(i,j,nzc-1),grid->getZC(i,j,nzc-1));
  ...
  if (vct->getZleft_neighbor()==MPI_PROC_NULL){
    for (int i=0; i < nxc;i++)
      for (int j=0; j < nyc;j++){
        ...
        Bxc[i][j][0] = amrvac_obj->getbx_mhd(grid->getXC(i,j,0),grid->getYC(i,j,0),grid->getZC(i,j,0));
        Bxc[i][j][1] = amrvac_obj->getbx_mhd(grid->getXC(i,j,1),grid->getYC(i,j,1),grid->getZC(i,j,1));
        Bxc[i][j][2] = amrvac_obj->getbx_mhd(grid->getXC(i,j,2),grid->getYC(i,j,2),grid->getZC(i,j,2));

        Byc[i][j][0] = amrvac_obj->getby_mhd(grid->getXC(i,j,0),grid->getYC(i,j,0),grid->getZC(i,j,0));
        Byc[i][j][1] = amrvac_obj->getby_mhd(grid->getXC(i,j,1),grid->getYC(i,j,1),grid->getZC(i,j,1));
        Byc[i][j][2] = amrvac_obj->getby_mhd(grid->getXC(i,j,2),grid->getYC(i,j,2),grid->getZC(i,j,2));

        Bzc[i][j][0] = amrvac_obj->getbz_mhd(grid->getXC(i,j,0),grid->getYC(i,j,0),grid->getZC(i,j,0));
  ...
  if (vct->getYright_neighbor() == MPI_PROC_NULL) {
    for (int i = 0; i < nxc; i++)
      for (int k = 0; k < nzc; k++) {
        //Bxc[i][nyc - 1][k] = B0x * tanh((grid->getYC(i, nyc - 1, k) - Ly / 2) / delta);
        Bxc[i][nyc-1][k] = amrvac_obj->getbx_mhd(grid->getXC(i,nyc-1,k),grid->getYC(i,nyc-1,k),grid->getZC(i,nyc-1,k)); 
        Bxc[i][nyc-2][k] = amrvac_obj->getbx_mhd(grid->getXC(i,nyc-2,k),grid->getYC(i,nyc-2,k),grid->getZC(i,nyc-2,k));
        Bxc[i][nyc-3][k] = amrvac_obj->getbx_mhd(grid->getXC(i,nyc-3,k),grid->getYC(i,nyc-3,k),grid->getZC(i,nyc-3,k));

        Byc[i][nyc-1][k] = amrvac_obj->getby_mhd(grid->getXC(i,nyc-1,k),grid->getYC(i,nyc-1,k),grid->getZC(i,nyc-1,k));

        Bzc[i][nyc-1][k] = amrvac_obj->getbz_mhd(grid->getXC(i,nyc-1,k),grid->getYC(i,nyc-1,k),grid->getZC(i,nyc-1,k));
        Bzc[i][nyc-2][k] = amrvac_obj->getbz_mhd(grid->getXC(i,nyc-2,k),grid->getYC(i,nyc-2,k),grid->getZC(i,nyc-2,k));
        Bzc[i][nyc-3][k] = amrvac_obj->getbz_mhd(grid->getXC(i,nyc-3,k),grid->getYC(i,nyc-3,k),grid->getZC(i,nyc-3,k));

        //cout << " kdm inside fixBAMRVAC i,j,k" << i << " " << nyc-1 << " " << k << " " << Bxc[i][nyc-1][k] << " " << Byc[i][nyc-1][k] << endl;
  ...
  if (vct->getYleft_neighbor() == MPI_PROC_NULL) {
    for (int i = 0; i < nxc; i++)
      for (int k = 0; k < nzc; k++) {
        //Bxc[i][0][k] = B0x * tanh((grid->getYC(i, 0, k) - Ly / 2) / delta);
        Bxc[i][0][k] = amrvac_obj->getbx_mhd(grid->getXC(i,0,k),grid->getYC(i,0,k),grid->getZC(i,0,k)); 
        Bxc[i][1][k] = amrvac_obj->getbx_mhd(grid->getXC(i,1,k),grid->getYC(i,1,k),grid->getZC(i,1,k));
        Bxc[i][2][k] = amrvac_obj->getbx_mhd(grid->getXC(i,2,k),grid->getYC(i,2,k),grid->getZC(i,2,k));

        Byc[i][0][k] = amrvac_obj->getby_mhd(grid->getXC(i,0,k),grid->getYC(i,0,k),grid->getZC(i,0,k));

        Bzc[i][0][k] = amrvac_obj->getbz_mhd(grid->getXC(i,0,k),grid->getYC(i,0,k),grid->getZC(i,0,k));
        Bzc[i][1][k] = amrvac_obj->getbz_mhd(grid->getXC(i,1,k),grid->getYC(i,1,k),grid->getZC(i,1,k));
        Bzc[i][2][k] = amrvac_obj->getbz_mhd(grid->getXC(i,2,k),grid->getYC(i,2,k),grid->getZC(i,2,k));
         
        //cout << " kdm inside fixBAMRVAC i,k " << i << " " << nyc-1 << " " << k << " " << Bxc[i][0][k] << " " << Byc[i][0][k] << endl;
  ...
  if (vct->getXright_neighbor() == MPI_PROC_NULL) {
    for (int j=0; j < nyc;j++)
      for (int k=0; k < nzc;k++){
        ...
        Bxc[nxc-1][j][k] = amrvac_obj->getbx_mhd(grid->getXC(nxc-1,j,k),grid->getYC(nxc-1,j,k),grid->getZC(nxc-1,j,k));

        Byc[nxc-1][j][k] = amrvac_obj->getby_mhd(grid->getXC(nxc-1,j,k),grid->getYC(nxc-1,j,k),grid->getZC(nxc-1,j,k));
        Byc[nxc-2][j][k] = amrvac_obj->getby_mhd(grid->getXC(nxc-2,j,k),grid->getYC(nxc-2,j,k),grid->getZC(nxc-2,j,k));
        Byc[nxc-3][j][k] = amrvac_obj->getby_mhd(grid->getXC(nxc-3,j,k),grid->getYC(nxc-3,j,k),grid->getZC(nxc-3,j,k));

        Bzc[nxc-1][j][k] = amrvac_obj->getbz_mhd(grid->getXC(nxc-1,j,k),grid->getYC(nxc-1,j,k),grid->getZC(nxc-1,j,k));
        Bzc[nxc-2][j][k] = amrvac_obj->getbz_mhd(grid->getXC(nxc-2,j,k),grid->getYC(nxc-2,j,k),grid->getZC(nxc-2,j,k));
        Bzc[nxc-3][j][k] = amrvac_obj->getbz_mhd(grid->getXC(nxc-3,j,k),grid->getYC(nxc-3,j,k),grid->getZC(nxc-3,j,k));
  ...
  if (vct->getXleft_neighbor()==MPI_PROC_NULL){
    for (int j=0; j < nyc;j++)
      for (int k=0; k < nzc;k++){
        ...
        Bxc[0][j][k] = amrvac_obj->getbx_mhd(grid->getXC(0,j,k),grid->getYC(0,j,k),grid->getZC(0,j,k));

        Byc[0][j][k] = amrvac_obj->getby_mhd(grid->getXC(0,j,k),grid->getYC(0,j,k),grid->getZC(0,j,k));
        Byc[1][j][k] = amrvac_obj->getby_mhd(grid->getXC(1,j,k),grid->getYC(1,j,k),grid->getZC(1,j,k));
        Byc[2][j][k] = amrvac_obj->getby_mhd(grid->getXC(2,j,k),grid->getYC(2,j,k),grid->getZC(2,j,k));

        Bzc[0][j][k] = amrvac_obj->getbz_mhd(grid->getXC(0,j,k),grid->getYC(0,j,k),grid->getZC(0,j,k));
        Bzc[1][j][k] = amrvac_obj->getbz_mhd(grid->getXC(1,j,k),grid->getYC(1,j,k),grid->getZC(1,j,k));
        Bzc[2][j][k] = amrvac_obj->getbz_mhd(grid->getXC(2,j,k),grid->getYC(2,j,k),grid->getZC(2,j,k));
...
//#######################kdm addition 24-02-2016####################################################################################
/*! initiliaze EM from AMRVAC data */
void EMfields3D::initAMRVAC(VirtualTopology3D * vct, Grid * grid, Collective *col) 
{
  int mhdsuccess;
  double xx,yy,zz;
  amrvac_interf* amrvac_obj = new amrvac_interf();
  mhdsuccess = amrvac_obj->init_sim_box();
  mhdsuccess = amrvac_obj->fields_from_amrvac();
  mhdsuccess = amrvac_obj->vel_from_amrvac();

  //filling up the node values by linear interpolation from mhd data
  for (int i=0; i<nxn; i++)
    for (int j=0; j<nyn; j++)
      for (int k=0; k<nzn; k++)
      {
        ...
        yy = grid->getYN(i,j,k);
        zz = grid->getZN(i,j,k);
        Bxn[i][j][k] = amrvac_obj->getbx_mhd(xx,yy,zz);
        Byn[i][j][k] = amrvac_obj->getby_mhd(xx,yy,zz);
        Bzn[i][j][k] = amrvac_obj->getbz_mhd(xx,yy,zz);
        Ex[i][j][k]  = amrvac_obj->getex_mhd(xx,yy,zz);
        Ey[i][j][k]  = amrvac_obj->getey_mhd(xx,yy,zz);
        Ez[i][j][k]  = amrvac_obj->getez_mhd(xx,yy,zz);
        for (int is = 0; is < ns; is++) {
          rhons[is][i][j][k] = amrvac_obj->getrho_mhd(xx,yy,zz)/FourPI;
        } 
      }

  //loop over cell centers and fill in magnetic fields and rho
  for (int i=0; i < nxc; i++)
    for (int j=0; j < nyc; j++)
      for (int k=0; k < nzc; k++)
      {
        ...
        yy = grid->getYC(i,j,k);
        zz = grid->getZC(i,j,k);
        Bxc[i][j][k] = amrvac_obj->getbx_mhd(xx,yy,zz);
        Byc[i][j][k] = amrvac_obj->getby_mhd(xx,yy,zz);
        Bzc[i][j][k] = amrvac_obj->getbz_mhd(xx,yy,zz);
        for (int is = 0; is < ns; is++) {
          rhocs[is][i][j][k] = amrvac_obj->getrho_mhd(xx,yy,zz)/FourPI;
          //setting rhons by hand just for testing
          //rhons[is][i][j][k] = amrvac_obj->rho_mhd[i][j][k];
        }
        //col->setFluidFieldsCenter(&Ex[i][j][k],&Ey[i][j][k],&Ez[i][j][k],
        //    &Bxc[i][j][k],&Byc[i][j][k],&Bzc[i][j][k],i,j,k);
        //Bxc[i][j][k] = amrvac_obj->Bx_mhd[i][j][k];
        //Byc[i][j][k] = amrvac_obj->By_mhd[i][j][k];
        //Bzc[i][j][k] = amrvac_obj->Bz_mhd[i][j][k];
      }
 
  //double dummy = rhocs[0][12][25][0]; 
  //cout << "kdm  rhocs " << dummy << endl; 
  //cout << "kdm grid->getXN= on proc" << vct->getCartesian_rank() << " " << grid->getYN(0, 0, 0) << endl;

  //interpolate from cell centers to nodes (corners of cells)
  //for (int is=0 ; is<ns; is++)
  //  grid->interpC2N(rhons[is],rhocs[is]); 
  
  //dummy = rhons[0][32][2][0]; 
  //cout << "kdm  nxc, nyc, rhons " << nxc << nyc << nxn << nyn << dummy << endl; 
...
//kdm addition 12-05-2016
/*! AMRVAC boundary conditions: LEFT wall */
void EMfields3D::perfectAMRVACLeft(double ***imageX, double ***imageY, double ***imageZ, double ***vectorX, double ***vectorY, double ***vectorZ, int dir, Grid * grid) {

  cout << " kdm inside perfectAMRVACLeft " << endl;
  ...
  switch(dir){
    ...
    case 2: // boundary condition on Z-DIRECTION
      ...
      for (int i=1; i <  nxn-1;i++)
        for (int j=1; j <  nyn-1;j++){
          imageX[i][j][1] = vectorX[i][j][1];
          imageY[i][j][1] = vectorY[i][j][1];//kdm correction, it was vectorX earlier
...
//kdm addition 12-05-2016
/*! AMRVAC boundary conditions: RIGHT wall */
void EMfields3D::perfectAMRVACRight(double ***imageX, double ***imageY, double ***imageZ, double ***vectorX, double ***vectorY, double ***vectorZ, int dir, Grid * grid) {

  //cout << " kdm inside perfectAMRVACRight " << endl;
    ...
    case 2: // boundary condition on Z-DIRECTION
      ...
      for (int i=1; i <  nxn-1;i++)
        for (int j=1; j <  nyn-1;j++){
          ...
          imageY[i][j][nzn-2] = vectorY[i][j][nzn-2];//kdm correction, it was vectorX earlier
...
//kdm addition 06-05-2015
/*! Boundary conditions from AMRVAC: LEFT WALL */
void EMfields3D::perfectAMRVACLeftS(double ***vectorX, double ***vectorY, double ***vectorZ, Grid * grid, int dir) {

  //cout << " inside kdm perfectAMRVACLeftS " << endl;
  ...
  //xx = grid->getXN(8,23,1);
  //yy = grid->getYN(8,23,1);
  //zz = grid->getZN(8,23,1);
  //dval = amrvac_obj->getex_direct(xx,yy,zz);
  //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << nzn << " " << dval << endl ;
    ...
    case 0: //boundary condition on X-direction left
      for (int i=1; i < nyn-1;i++)
        for (int j=1; j < nzn-1;j++){
          ...
          yy = grid->getYN(1,i,j);
          zz = grid->getZN(1,i,j);
          vectorX[1][i][j] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[1][i][j] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[1][i][j] = amrvac_obj->getez_mhd(xx,yy,zz);
          cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[1][i][j] << " " << vectorY[1][i][j] << " " << vectorZ[1][i][j] << endl ;
        }
      break;
    case 1: //boundary condition on Y-direction LEFT
      for (int i=1; i < nxn-1;i++)
        for (int j=1; j < nzn-1;j++){
          ...
          yy = grid->getYN(i,1,j);
          zz = grid->getZN(i,1,j);
          vectorX[i][1][j] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[i][1][j] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[i][1][j] = amrvac_obj->getez_mhd(xx,yy,zz);
          //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[i][1][j] << " " << vectorY[i][1][j] << " " << vectorZ[i][1][j] << endl ;
        } 
      break;
    case 2: //boundary condition on Z-direction LEFT
      for (int i=1; i < nxn-1;i++)
        for (int j=1; j < nyn-1;j++){
          ...
          yy = grid->getYN(i,j,1);
          zz = grid->getZN(i,j,1);
          vectorX[i][j][1] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[i][j][1] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[i][j][1] = amrvac_obj->getez_mhd(xx,yy,zz); 
          //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[i][j][1] << " " << vectorY[i][j][1] << " " << vectorZ[i][j][1] << endl ;
        }
      break;
  }
...
//kdm addition 11-05-2015
/*! Boundary conditions from AMRVAC: RIGHT WALL */
void EMfields3D::perfectAMRVACRightS(double ***vectorX, double ***vectorY, double ***vectorZ, Grid * grid, int dir) {

  //cout << " inside kdm perfectAMRVACRightS " << endl;
  ...
  switch(dir){
    case 0: //boundary condition on X-direction right
      for (int i=1; i < nyn-1;i++)
        for (int j=1; j < nzn-1;j++){
          xx = grid->getXN(nxn-2,i,j);
          yy = grid->getYN(nxn-2,i,j);
          zz = grid->getZN(nxn-2,i,j);
          vectorX[nxn-2][i][j] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[nxn-2][i][j] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[nxn-2][i][j] = amrvac_obj->getez_mhd(xx,yy,zz);
          //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[nxn-2][i][j] << " " << vectorY[nxn-2][i][j] << " " << vectorZ[nxn-2][i][j] << endl ;
        }
      break;
    case 1: //boundary condition on Y-direction RIGHT
      for (int i=1; i < nxn-1;i++)
        for (int j=1; j < nzn-1;j++){
          xx = grid->getXN(i,nyn-2,j);
          yy = grid->getYN(i,nyn-2,j);
          zz = grid->getZN(i,nyn-2,j);
          vectorX[i][nyn-2][j] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[i][nyn-2][j] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[i][nyn-2][j] = amrvac_obj->getez_mhd(xx,yy,zz);
          //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[i][nyn-2][j] << " " << vectorY[i][nyn-2][j] << " " << vectorZ[i][nyn-2][j] << endl ;
        } 
      break;
    case 2: //boundary condition on Z-direction RIGHT
      for (int i=1; i < nxn-1;i++)
        for (int j=1; j < nyn-1;j++){
          xx = grid->getXN(i,j,nzn-2);
          yy = grid->getYN(i,j,nzn-2);
          zz = grid->getZN(i,j,nzn-2);
          vectorX[i][j][nzn-2] = amrvac_obj->getex_mhd(xx,yy,zz);
          vectorY[i][j][nzn-2] = amrvac_obj->getey_mhd(xx,yy,zz);
          vectorZ[i][j][nzn-2] = amrvac_obj->getez_mhd(xx,yy,zz); 
          //cout << " inside kdm AMRVAC BC xx, yy, zz, dval = " << xx << " " << yy << " " << vectorX[i][j][nzn-2] << " " << vectorY[i][j][nzn-2] << " " << vectorZ[i][j][nzn-2] << endl ;
        }
      break;
  }
...
double EMfields3D::getEenergy(void) {
  ...
  MPI_Allreduce(&localEenergy, &totalEenergy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_IPIC3D);
  return (totalEenergy);
...
double EMfields3D::getBenergy(void) {
  ...
  MPI_Allreduce(&localBenergy, &totalBenergy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_IPIC3D);
  return (totalBenergy);
}
