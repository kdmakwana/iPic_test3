...
void Collective::ReadInput(string inputfile) {
  ...
  if(!RESTART1) {
    ...
    //kdm addition 18-02-2016################################################################################################################
    int mhdsuccess;
    amrvac_interf* amrvac_obj = new amrvac_interf();
    mhdsuccess = amrvac_obj->init_sim_box(); 
    cout << "kdm xl,xh,yl,yh " << amrvac_obj->xl_mhd << amrvac_obj->xh_mhd << amrvac_obj->yl_mhd << amrvac_obj->yh_mhd << endl;
    cout << "kdm dx_pic, dy_pic " << amrvac_obj->dx_mhd << amrvac_obj->dy_mhd << endl;
    nxc = amrvac_obj->nx_mhd-3;
    nyc = amrvac_obj->ny_mhd-3;
    nzc = amrvac_obj->nz_mhd;
    Lx = amrvac_obj->xh_mhd-amrvac_obj->xl_mhd;   
    Ly = amrvac_obj->yh_mhd-amrvac_obj->yl_mhd;   
    //mhdsuccess = amrvac_obj->fields_from_amrvac(); 
    cout << " kdm nxc in collective is " << nxc << " " << nyc << " " << nzc << endl; 
    delete amrvac_obj;
    //end of kdm addition####################################################################################################################
