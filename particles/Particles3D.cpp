...
//################################kdm addition 02-03-2016#################################################################
/** Maxellian random velocity and uniform spatial distribution */
void Particles3D::maxwellian_amrvac(Grid* grid,Field* EMf,VirtualTopology3D* vct, Collective* col, int spc)
{

  int mhdsuccess, opp_s;
  amrvac_interf* amrvac_obj = new amrvac_interf();
  mhdsuccess = amrvac_obj->init_sim_box();
  mhdsuccess = amrvac_obj->vel_from_amrvac();
  double rhomhd, presmhd, gamdum, lightc, corrk;  
  //double dummy = amrvac_obj->uy_mhd[12][23][0];
  //cout << "inside particles kdm uy= "<< dummy <<endl;
  
  /* initialize random generator with different seed on different processor */
  srand(vct->getCartesian_rank()+2);
  double harvest, ufx, ufy, ufz, jfx, jfy, jfz;
  double prob, theta, sign, qomarr[2], bigM, prefac, denom;
  qomarr[1] = col->getQOM(1);//ions
  qomarr[0] = col->getQOM(0);//electrons
  bigM = 1.0+abs(qomarr[1]/qomarr[0]);//total mass for mi=1 
  cout << "inside particles kdm "<< " bigM= " << bigM << " " <<endl;
	  ...
	  for (int jj=0; jj < npcely; jj++)
	    for (int kk=0; kk < npcelz; kk++){
	      ...
	      q[counter] = (qom/fabs(qom))*(EMf->getRHOcs(i,j,k,ns)/npcel)*(1.0/grid->getInvVOL());
              ...
              corrk = npcel*(grid->getInvVOL());              

	      //getting the mhd quantities
	      ufx = amrvac_obj->getufx_mhd(x[counter],y[counter],z[counter]);
	      ufy = amrvac_obj->getufy_mhd(x[counter],y[counter],z[counter]);
	      ufz = amrvac_obj->getufz_mhd(x[counter],y[counter],z[counter]);
	      jfx = amrvac_obj->getjfx_mhd(x[counter],y[counter],z[counter]);
	      jfy = amrvac_obj->getjfy_mhd(x[counter],y[counter],z[counter]);
	      jfz = amrvac_obj->getjfz_mhd(x[counter],y[counter],z[counter]);
              rhomhd = amrvac_obj->getrho_mhd(x[counter],y[counter],z[counter]);

              //the following only works for 2-species, qi=-qe, ni=ne, mi=1 plasma
              opp_s = 1-ns;
              //cout << "kdm ns and opp_s = " << ns << " " << opp_s << endl; 
              prefac = 1.0+abs(qomarr[ns]/qomarr[opp_s]); 
              denom = qomarr[opp_s]-qomarr[ns];
              //cout << "kdm ns,prefac and denom = " << ns << " " << prefac << " " << denom << endl; 
              u0 = prefac*((qomarr[opp_s]*ufx)-(jfx/(bigM*rhomhd)))/denom;
              v0 = prefac*((qomarr[opp_s]*ufy)-(jfy/(bigM*rhomhd)))/denom;
              w0 = prefac*((qomarr[opp_s]*ufz)-(jfz/(bigM*rhomhd)))/denom;
              //cout << "kdm ns,q,jfz,w0 = " << ns << " " << q[counter] << " " << jfz << " " << w0 << endl; 

	      harvest =   rand()/(double)RAND_MAX;
	      prob  = sqrt(-2.0*log(1.0-.999999*harvest));
	      harvest =   rand()/(double)RAND_MAX;
	      theta = 2.0*M_PI*harvest;
	      //u0 = amrvac_obj->getVix_mhd(x[counter],y[counter],z[counter]);
	      //v0 = amrvac_obj->getViy_mhd(x[counter],y[counter],z[counter]);
	      //w0 = amrvac_obj->getViz_mhd(x[counter],y[counter],z[counter]);

	      //converting momentum into mean velocity
	      //lightc = 1.0;
              //rhomhd = amrvac_obj->getrho_mhd(x[counter],y[counter],z[counter]);
              //gamdum = sqrt(rhomhd*rhomhd+(u0*u0+v0*v0+w0*w0)/(lightc*lightc));
              //u0 = u0/gamdum;
              //v0 = v0/gamdum;
              //w0 = w0/gamdum;
 
              //getting the thermal velocity
              presmhd = amrvac_obj->getprs_mhd(x[counter],y[counter],z[counter]);
              double uthMHD = sqrt(presmhd*abs(qomarr[ns])/rhomhd);
	      //if(uthMHD > uth) uthMHD = uth;
              uthMHD = 0.0*uthMHD;  
 
	      //if(q[counter]<0){
	        //u0 = u0 - EMf->getJx0(i,j,k) / EMf->getRHOns(i,j,k,ns);
		//v0 = v0 - EMf->getJy0(i,j,k) / EMf->getRHOns(i,j,k,ns);
		//w0 = w0 - EMf->getJz0(i,j,k) / EMf->getRHOns(i,j,k,ns);
	      //}

	      //double uthMHD = sqrt(fabs(qom) *col->getT_ext(x[counter],y[counter],z[counter]));
	      //cout << "inside particles kdm "<< " ux= " << u0<< " uy= "<< v0 <<endl;
	      //double uthFluid = uth;
	      //if(uthMHD > uth) {
	      //  double uthFluid=uthMHD;
	      //}

	      //u[counter] = 0.01;
              //v[counter] = 0.0;
              //w[counter] = 0.0;
              u[counter] = u0 + uthMHD*prob*cos(theta);
	      v[counter] = v0 + uthMHD*prob*sin(theta);
	      harvest =   rand()/(double)RAND_MAX;
	      prob  = sqrt(-2.0*log(1.0-.999999*harvest));
	      harvest =   rand()/(double)RAND_MAX;
	      theta = 2.0*M_PI*harvest;
	      w[counter] = w0 + uthMHD*prob*cos(theta);
...
//#pragma simd                    // this just slows things down (why?)
  ...
  for (long long rest = 0; rest < nop; rest++) {
    ...
    //kdm addition just checking
    if (rest <= 100) {
      //cout << "inside mover sub, ns,yp,wp = " << ns << " " << yp << " " << wp << endl; 
    }
