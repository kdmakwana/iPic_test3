...
double Particles3Dcomm::getKe() {
  ...
  MPI_Allreduce(&localKe, &totalKe, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_IPIC3D);
  return (totalKe);
}
...
double Particles3Dcomm::getP() {
  ...
  MPI_Allreduce(&localP, &totalP, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_IPIC3D);
  return (totalP);
}
...
double Particles3Dcomm::getMaxVelocity() {
  ...
  MPI_Allreduce(&localVel, &maxVel, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_IPIC3D);
  return (maxVel);
}
...
unsigned long *Particles3Dcomm::getVelocityDistribution(int nBins, double maxVel) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, f, nBins, MPI_LONG_LONG, MPI_SUM, MPI_COMM_IPIC3D);
