...
int c_Solver::Init(int argc, char **argv) {
  ...
  vct->setup_vctopology(MPI_COMM_IPIC3D);
  ...
  if (col->getSolInit()) {
    ...
    else if (col->getCase()=="AMRVAC")    EMf->initAMRVAC(vct,grid,col);//kdm addition 24-02-2016
    else {
      if (myrank==0) {
        cout << " =========================================================== " << endl;
  ...
  if (col->getSolInit()) {
    ...
    if (restart == 0) {
      ...
      for (int i = 0; i < ns; i++)
        ...
        else if (col->getCase()=="AMRVAC")    part[i].maxwellian_amrvac(grid,EMf,vct,col,i);//kdm addition 07-03-2016
        else                                  part[i].maxwellian(grid, EMf, vct);
